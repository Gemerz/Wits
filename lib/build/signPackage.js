...
if (util.isFileExist(CANON_PATH)) {
    ExclusiveCanonicalization = require('../build/exclusive-canonicalization')
...
const PUBLIC_SIGNATURE = path.join(
    util.WITS_BASE_PATH,
    '../',
    'container',
    'signature1.xml'
);
const PUBLIC_SIGNATURE2 = path.join(
    util.WITS_BASE_PATH,
    '../',
    'container',
    'signature2.xml'
...
function createSignatureXML(signatureId, destfile) {
    try {
        ...
        if (signatureId === 'AuthorSignature') {
            ...
            var distributorKeyFile =
                p12ToPem.ACTIVE_PEM_FILE.DISTRIBUTOR_KEY_FILE;
            if (destfile.indexOf('signature2') >= 0) {
                distributorKeyFile =
                    p12ToPem.ACTIVE_PEM_FILE.DISTRIBUTOR2_KEY_FILE;
            }
        ...
        console.warn(`Failed to createSignatureXML ${e}`);
        throw e;
    }
}
